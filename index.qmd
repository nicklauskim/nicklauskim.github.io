---
title: "Nicklaus Kim"
page-layout: full
format:
  html:
    toc: false
---

<style>
/* Layout: two columns */
.page-columns {
  display: flex;
  align-items: flex-start;
  gap: 2rem;
}

/* Left content column */
.content {
  flex: 5;
}

/* Right sidebar */
.column-margin {
  flex: 2;
}

/* Sidebar card styling */
.profile-card {
  /* position: sticky; */
  top: 2rem;
  padding: 1rem;
  border-radius: 12px;
  background-color: #f8f9fa;
  /* text-align: center; */
}

.profile-card img {
  width: 100%;
  max-width: 240px;
  border-radius: 12px;
  display: block;
  margin: 0 auto 0.75rem auto;
}

.profile-contact {
  font-size: 0.9rem;
  /* text-align: center; */
  margin-top: 0.5rem;
}

/* remove newline space before bulleted list */
p + ul {
  margin-top: 0;
}

/* Mobile responsiveness */
@media (max-width: 900px) {
  .page-columns {
    display: block;
  }
  .profile-card {
    position: static;
    margin-bottom: 2rem;
  }
}
</style>

<div class="page-columns">

<!-- LEFT COLUMN -->
<div class="content">


Hello, I'm Nick. Welcome to my professional and academic portfolio website.

I completed my M.S. in Statistics at UCLA and am currently planning to go back to school for a PhD in Statistics beginning in Fall 2026. 

In my research, I'm interested in a wide array of ideas lying at the intersection of statistics and applied mathematics. My primary interests lie in Bayesian modeling and inference, with particular emphasis on inverse problems, uncertainty quantification, and Bayesian data assimilation. I am also interested in probabilistic modeling frameworks that incorporate mathematical structure, including stochastic processes, spatial and spatiotemporal statistics, and functional data analysis. 

I have experience in building tools like data dashboards and statistical learning models to help uncover meaningful insights about data, including performing statistical/machine learning research in academia. I try to maintain and continually expand a diverse, up-to-date set of data analysis and research skills; in particular I specialize in Python, R, SQL (PostgreSQL), and frameworks like Dash and Flask for data dashboard applications and visual storytelling.

I'm also very passionate about statistics and mathematics education. I produce instructive videos and articles explaining core concepts, discussing interesting historical uses of statistics, and exploring more niche advanced topics. I also work as a tutor part-time, which gives me the opportunity to help high school and college students grow their mastery (and hopefully appreciation!) of stats and math.
<!-- (Check out my blog [here](https://nicklauskim.github.io/blog).) -->

(Note: I am continuously trying to consolidate all of my work here—which often feels scattered all over the place—on this website, so some things may be temporarily missing or incomplete. Work is ongoing! )

> You can find quick links to my statistics research, data science projects, and other work I've done on my [Projects](https://nicklauskim.github.io/projects) page and at my [GitHub](https://github.com/nicklauskim).


### Projects

* [Data-Centric Machine Learning (Synthetic Data) Research](projects/all/ms-thesis/index.qmd)
    + An investigation into auditing the true privacy benefits of the use of synthetic data in sensitive domains such as healthcare, incorporating generative models for producing datasets and adversarial attack frameworks to assess (differential) privacy versus downstream model utility.
* [Bayesian Inverse Problems for ODE Systems under Model Misspecification](projects/all/bayesian-ode/index.qmd)
    + Theoretical and applied investigations into the effects of Bayesian model misspecification on statistical inference in nonlinear ordinary differential equations and dynamical system models (e.g., SIR, Lotka–Volterra, and their extensions) using Python/PyMC.
    <!-- + Bayesian Inverse Problems, Uncertainty Quantification, Functional Data Analysis -->
* [Founding Fathers Correspondence Network](projects/all/project-1775/index.qmd)
    + An ongoing initiative to analyze and gain new data-driven insights into early writings and correspondence of early American figures from the founding era via contextual and spatiotemporal analysis of letter frequencies, knowledge networks and other visualizations.
<!-- * [NBA Statistics Data Dashboard](projects/all/nba-dash/index.qmd)
    + A dynamic data visualization platform built with Dash and Plotly, incorporating a rich database of official NBA statistics served with a FastAPI backend and allowing for deep exploration of play-by-play and other detailed data as well as customizable filtering, plotting, and generation of video clips. -->


### Notes

* [Statistical Inference](notes/statistical-inference/index.qmd)
* [Bayesian Data Analysis](notes/bayesian-data-analysis/index.qmd)
* [Introduction to Statistics](notes/intro-stats/index.qmd)


<hr>
<br>
<p>This website was built using Quarto.</p> 

</div> <!-- end left column -->


<!-- RIGHT SIDEBAR -->
<div class="column-margin">

<div class="profile-card">
  <img src="assets/img/me.png" alt="Nicklaus Kim" oncontextmenu="return false;">
    **Email:** [nickausjkim@gmail.com](mailto:nickausjkim@gmail.com)<br><br>
    <!-- <strong>GitHub:</strong> <a href="https://github.com/nicklauskim">github.com/nicklauskim</a><br> -->
    <!-- <p><strong>LinkedIn:</strong> <a href="#">linkedin.com/in/...</a></p> --> 
</div>

</div> <!-- end column-margin --> 


</div> <!-- end page-columns --> 
